...
def train(config=None):
    with wandb.init(config=config):
        ...
        if config.depth == 4:
            opt.FeatureExtraction = "VGG_4"
        elif config.depth == 5:
            opt.FeatureExtraction = "VGG_5"
        elif config.depth == 6:
            opt.FeatureExtraction = "VGG_6"
        elif config.depth == 7:
            opt.FeatureExtraction = "VGG_7"
        ...
        while(True):
            ...
            if (iteration + 1) % opt.valInterval == 0 or iteration == 0: # To see training progress, we also conduct validation when 'iteration == 0' 
                ...
                with open(f'./saved_models/{opt.exp_name}/log_train.txt', 'a') as log:
                    ...
                    wandb.log({"Valid loss": valid_loss, "Current accuracy": current_accuracy})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_max_length', type=int, default=9, help='maximum-label-length')
